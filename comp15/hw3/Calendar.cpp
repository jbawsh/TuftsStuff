...
//  Calendar.cpp
//
// Contains implementations for the calendar class which holds days of
// events in a dynamic array.
//
// Made by Chris Gregg
...
Day *Calendar::getDay(date startDate) {
        ...
        for (index = 0; index < dayCount; index++) {
                if (isSame(startDate, dayList[index].getDate())) {
                        sameDay = &dayList[index];
                        break;
                }
        }
        return sameDay;
...
Day *Calendar::insertDay(date startDate) {
        ...
        int index;
        if (getDay(startDate) != NULL) {
                return getDay(startDate);
        }
        for (index = 0; index < dayCount; index++) {
                if (isBefore(startDate, dayList[index].getDate())) break;
        }
        for (int i = dayCount; i > index; i--) {
...
// incriments the array printing out each day and then has one extra line of
// space at the end.
...
// Compares two dates to see if one comes before another one and returns
// a bool with the result.
 bool Calendar::isBefore(date someDate, date otherDate)
 {
         if (someDate.year < otherDate.year) {
                 return true;
         } else if (someDate.year == otherDate.year &&
                    someDate.month < otherDate.month) {
                 return true;
         } else if (someDate.year == otherDate.year &&
                    someDate.month == otherDate.month &&
                    someDate.day < otherDate.day) {
                 return true;
         } else {
                 return false;
         }
...
// Compares two dates to see if they are the same and returns a bool with
// the result.
 bool Calendar::isSame(date someDate, date otherDate)
 {
         if (someDate.year == otherDate.year &&
             someDate.month == otherDate.month &&
             someDate.day == otherDate.day) {
