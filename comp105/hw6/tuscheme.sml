...
(* fun typeof (e, gamma, delta) =
  let
      fun ty (LITERAL (NIL)) = FORALL(["'a"], lissttype tyvarA))
      |   ty (LITERAL (BOOL x)) = booltype
      |   ty (LITERAL (NUM x)) = inttype
      |   ty (LITERAL (SYM x)) = symtype
      |   ty (LITERAL (PAIR (x, xs))) = if eqtype(listtype (ty (LITERAL x)),
                                                  ty(LITERAL xs)) then
                                           ty(LITERAL b)
                                        else
                                           raise TypeError("Pair not matching")
      |   ty (LITERAL (CLOSURE (x, env))) = raise TypeError ("closure reached")
      |   ty (LITERAL (PRIMITIVE x)) = raise TypeError ("primitive reached")
      ...
      |   ty (SET (x, e) =
             let val xtau = ty (VAR x)
                 val etau = ty e
             in
                if eqType (xtau, etau) then
                   xtau
                else
                   raise TypeError ("Set var " ^ x ^ " of type " ^
                                    typeString xtau ^ " to val of type " ^
                                    typeString etau)
             end                       
      ...
      |   ty (IFX (e1, e2, e3)) =
             ...
             in if eqType (tau1, booltype) then
                   if eqType (tau2, tau3) then
                      ...
                      raise TypeError ("In if, true branch is type " ^
                                       typeString tau2 ^
                                       " but false branch has type " ^
                                       typeString tau3)
                else
                   raise TypeError ("Condition in if is type " ^
                                    typeString tau1 ^
                                    " but should be bool")
          end
      ...
      |   ty (WHILEX (e1, e2)) =
             ...
             in
                if eqType (tau1, booltype) then
                   ...
                   raise TypeError ("Condition in while should be bool, not " ^
                                    typeString tau1)
             end
      ...
      |   ty (BEGIN es) =
             let val bodytypes = map ty es
                 fun last tau [] = tau
                 |   last tau (h::ts) = last h ts
             in  last unittypes bodytypes
             end

      |   ty (APPLY (fun, actuals)) =
             let val ftau = ty fun
                 val actypes = map ty actuals
             in
                case (tau)
                of (CONAPP (TYCON "function", [CONAPP (TYCON "argtuple", args),
                                               result]) =>
                   if eqTypes (actypes, args)
                      then result
                   else
                      raise TypeError("Arg types to function don't match")
                |  _ => raise TypeError("Didn't pass function to apply")
             end

      |   ty (LETX (LET, nexs, exp)) =
             let
                val names = map fst nexs
                val types = map ty (map snd nexs)
             in
                typeof (exp, (bindList (names, types, gamma)). delta)
             end

      |   ty (LETX (LETSTAR, nexs, exp)) =
             let
                val names = map fst nexs
                val types = map ty (map snd nexs)
                val etau = ty exp
             in
                if (null nexs) then
                   ...
                   typeof ((LETX(LETSTAR, (tl nexs), exp)), 
                                          (bind ((hd names), ((ty (hd types))),
                                                gamma)), delta)
             end
                                                 
      |   ty (LAMBDA lexp) =
             let
                val names = map fst (fst lexp)
                val types = map snd (fst lexp)
             in funtype (types, typeof(snd lexp, 
                         bindList(names, types, gamma)), delta)
             end

      |   ty (TYLAMBDA (names, e) = raise TypeError ("No idea")

      |   ty (TYAPPLY (exp, tyexs)) =
             instantiate((ty exp), tyexs, delta)
  in
      ty e
  end
