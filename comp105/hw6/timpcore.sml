      ...
      | ty (AGET (a, i))       = 
            ...
            in
                if eqType(itau, INTTY) then
                   case (atau) of ARRAYTY(etau) => etau
                   | (_) => raise TypeError ("Error " ^ typeString atau)
                else
                   raise TypeError ("Error " ^ typeString itau)
            end
      | ty (ASET (a, i, e))    =
            let
                val atau = ty a
            ...
            in
                if eqType (itau, INTTY) then
                   if eqType(atau, ARRAYTY(etau)) then
                      ...
                      raise TypeError ("Error " ^ typeString atau)
                else
                   raise TypeError ("Error " ^ typeString itau)
            end
      | ty (AMAKE (len, init)) =
            let
                ...
                val itau = ty init
            in
                if eqType(ltau, INTTY) then
                   ARRAYTY(itau)
                else
                   raise TypeError ("Error " ^ typeString ltau)
            end
      | ty (ALEN a)            =
            let
                val atau = ty a
            in
                if eqType(atau, INTTY) then
                   raise TypeError ("Error " ^ typeString atau)
                else if eqType(atau, BOOLTY) then
                   raise TypeError ("Error " ^ typeString atau)
                else
                   INTTY
            end
