...
Valuelist locallist(int count, Valuelist vl) {
        if (count > 1) {
                vl = mkVL(0, vl);
                return locallist(--count, vl);
        }
        return mkVL(0, vl);
}
...
/* make [[f]] the function denoted by [[e->u.apply.name]], or call [[error]] 43c */
            ...
            case USERDEF:
                /* apply [[f.u.userdef]] and return the result 44b */
                {
                    ...
                    int num_vars = lengthNL(f.u.userdef.locals);

                    Namelist  local_nl = f.u.userdef.locals;
                    Valuelist local_vl = locallist(num_vars, NULL);

                    checkargc(e, lengthNL(nl), lengthVL(vl));
                    return eval(f.u.userdef.body, globals, functions, 
                                mkValenv(nl, vl), mkValenv(local_nl, local_vl));
                }
            case PRIMITIVE:
