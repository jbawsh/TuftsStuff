...
void mark(void) {
        ...
        /* cycle, probably to do with root struct not really sure */
        visitroots();

        fprintf(stderr,"[GC stats: heap size %d live data %d ratio %.2f]\n"
                ,heapsize,nmarks - tempnmarks,
                (float)heapsize/(nmarks - tempnmarks));

 	if(ncollections % 10 == 0)
 	{
 		fprintf(stderr,"[Mem stats: heap size %d live data %d ratio %.2f]\n"
                        ,heapsize,nmarks - tempnmarks,
                        (float)heapsize/(nmarks - tempnmarks));
 	}
}

Value* next_empty(void)
{
        if(curpage == NULL)
                return NULL;
        
        while(1) {
                /* while still on current page */
                while (hp < heaplimit) {
                        if(hp->live == 0) {
                                /* found a spot */
                                return &(hp++)->v;
                        } else {
                                /* unmark it and check the next */
                                hp->live = 0;
                                hp++;
                        }
                }
                /* end of page so move to next */
                if(curpage->tl == NULL)
                        return NULL;
                else
...
Value* allocloc(void) {
        ...
        Value *val = NULL;
        nalloc++;
        /* pointer to next empty cell */
        val = next_empty();

        if(val != NULL) {
                return val;
        } else {
                /* if none found call mark and reset pointer */
                mark();

                /* if NULL then first alloc */
                if(pagelist != NULL) {
        ...
        /* find something or heap is full, does this count as growheap? */
        if (val != NULL) {
                return val;
        } else {
                addpage();
...
void printfinalstats(void) { 
  ...
  fprintf(stderr, "[Total GC work: %d collections marked %d objects; %.2f marks/allocations]\n",
          ncollections, nmarks, (float)nmarks/nalloc);
  assert(1); 
}
/* ms.c ((prototype)) 758f */
